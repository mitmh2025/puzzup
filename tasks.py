...
def export_users(c):
    c.run(
        "./manage.py dumpdata --format=yaml puzzle_editing.user --indent 4 >"
        " puzzle_editing/fixtures/auth.yaml"
    )
...
def export_users_from_prod(c):
    c.run(
        f"{HEROKU_MANAGE} dumpdata --format=yaml puzzle_editing.user --indent 4 >"
        " puzzle_editing/fixtures/auth.yaml"
    )
...
def export_prod(c):
    c.run(
        f"{HEROKU_MANAGE} dumpdata --format=yaml puzzle_editing --indent 4 >"
        " puzzle_editing/fixtures/data.yaml"
    )
...
def count_channels(c):
    c.run(
        f"{HEROKU_MANAGE} shell -c 'from puzzle_editing.discord_integration import"
        " get_client;c=get_client();print(len(c.get_channels_in_guild()))'"
    )
