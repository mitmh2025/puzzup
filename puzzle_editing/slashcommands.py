...
from puzzle_editing.discord import MsgPayload
from puzzle_editing.models import Puzzle
from puzzle_editing.view_helpers import external_puzzle_url
...
def slashCommandHandler(request):
    ...
    if payload["type"] == 1:
        # this is a ping
        return pingHandler()
    elif payload["type"] == 2 and payload["data"]["name"] == "up":
...
def archiveChannelHandler(request, payload):
    ...
    responseJson: dict[str, Any] = {"type": 4}
    puzzles = [
        {
            "name": p.name,
            "id": p.id,
    ...
    if len(puzzles) > 0:
        responsetext = (
            ":warning: This puzzle is already linked to the puzzle{} below (you cannot"
            " archive a linked channel):\n".format("" if len(puzzles) == 1 else "s")
        )
        responsetext += "\n".join(
            [
                "• <https://{}/puzzle/{}>: {} ||{}||".format(
