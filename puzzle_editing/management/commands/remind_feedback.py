...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["dry_run"]:
            print(
                "The following user(s) are yet to leave feedback on testsolve"
                " session(s):"
            )
            ...
            if options["dry_run"]:
                if missing_feedback:
                    ...
                    for sess in missing_feedback:
                        print(
                            f"Session #{sess.session.id} for puzzle"
                            f" (#{sess.session.puzzle.id}): {sess.session.puzzle.name}"
                        )
        ...
        if options["dry_run"]:
            print(
                f"\n{sent_count} user(s) would be reminded about"
                f" {reminded_count} outstanding testsolves"
            )
        else:
            print(
                f"Reminded {sent_count} user(s) about {reminded_count} outstanding"
                " testsolves"
            )
