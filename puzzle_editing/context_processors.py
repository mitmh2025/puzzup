...
def testsolving_allowed(request: HttpRequest):
    allowed_by_user = request.user.is_authenticated and request.user.has_perm('puzzle_editing.change_testsolvesession')
    allowed_by_site_setting = not SiteSetting.get_bool_setting('TESTSOLVING_DISABLED')
    return {
        "TESTSOLVING_ALLOWED": allowed_by_user or allowed_by_site_setting
    }
